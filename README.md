...
代理会把每次上游调用的 `usage` 记到 `~/.codex/jev-router/usage.jsonl`，
`jcodex-cost` 摊开给你看：

```bash
jcodex-cost              # 最近 7 天：输入/缓存命中/输出/思考/每步平均输入
jcodex-cost --days 1     # 只看今天
jcodex-cost --tail 20    # 逐条看最近 20 次调用
```
...
| 现象 | 数字 |
| --- | --- |
| 一句「只回答一个字」也要付的固定前缀 | ~10.7k tokens（系统提示+工具定义），加 MCP/插件约 +1.7k |
| 同一模型连续对话时的缓存命中率 | **88–97%** |
| **换模型**那一次的缓存命中率 | **0%**（前缀缓存按模型分区，换一次全废） |
| 一次工具循环里输入的增长 | 12.4k → 12.6k → 12.8k（历史累积） |

所以按影响排序的省钱手段：

1. **控制上下文规模**（最大）：每个工具步骤都要重发全部历史，上下文翻倍 = 每步成本翻倍。
   琐碎任务另开会话（`/new`）、及时 `/compact`，别让它把主会话撑大。
2. **别频繁换模型**：换一次 ≈ 白扔十几 k 全价输入。代理的**粘性路由**
   （`JEVPROXY_STICKY=1`，默认开）让升档立即、降档需连续 `JEVPROXY_DOWN_VOTES`（默认 2）
   轮都判低——于是切换发生在「阶段边界」而不是「每一轮」。
   实测效果：难→易的过渡轮，全价重算从 12,079 tokens 降到 2,133（缓存命中 24% → 87%）。
3. **难题用子 agent 承担**：主循环保持小上下文+便宜模型（缓存一直热），难题交给独立上下文的
   子 agent（Codex 里有 `default_subagent_model` 之类的键可配）。
4. **砍固定前缀**：关掉不用的 MCP server / 插件，每个步骤都省。
5. **reasoning effort 匹配档位**（已在做）：难任务里思考 token 才是输出成本的大头。
